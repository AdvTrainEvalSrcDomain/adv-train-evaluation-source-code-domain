...
replacement_map = defaultdict(lambda: -1)
...
def get_subnet_batch(model, x, y, doclabels_batch, authors_batch):
    ...
    for i, doclabel in enumerate(doclabels_batch):
        ...
        #max_loss = -1
        #max_prog = ''
        map_idx = replacement_map[doclabel.numpy().decode('utf8')]
        max_idx = random.randint(0, subnet_select_x.shape[0]-1) if map_idx == -1 else map_idx
        replacement_map[doclabel.numpy().decode('utf8')] = max_idx
...
def train_step(fullnet_batch_num, x, y, doclabels_batch, authors_batch, width_mult_list):
    ...
    grads = subtape.gradient(sub_loss, model.trainable_variables)
    accum_gradient = [(acum_grad+grad) for acum_grad, grad in zip(accum_gradient, grads)]
    K.clear_session()
...
if __name__ == '__main__':
    ...
    datasetpath = os.path.join(repo_path, "data", "trainnew_cpp_paper_augmented_fold1", "0")
    ...
    save_to = os.path.join(config_learning.learnmodelspath, 'comploss_checkpoints_' + random_str)
