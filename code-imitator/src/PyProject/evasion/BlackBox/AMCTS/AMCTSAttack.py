...
class AMonteCarloTreeSearch(BBAttackHandler):
    ...
    def perform_next_move(self, attackinstance: BBAttackInstance, outer_iteration_index: int):
        ...
        for ci in range(noofchildren):
            ...
            attackinstance.logger.logger.debug("\n\n")
            attackinstance.logger.logger.debug("~~>Next outer node id:" + str(winnernode.unique_node_id) + ", tf_id: " + winnernode.state.transformer_uniqueid + ", depth: " + str(winnernode.get_node_depth()))
    ...
    def mcts_core(self, attackinstance: BBAttackInstance, outer_iteration_index: int):
        ...
        for inner_iteration_index in range(0, self.mctssettings.max_inner_iterations):
            ...
            if succesfull_attackinstance is not None:
                node_depth = nodetoexplore.get_node_depth()
                attackinstance.logger.logger.debug("Selection node depth: " + str(node_depth))
                if node_depth > 0:
                    selnode_ids = ''
                    mynode = nodetoexplore
                    while mynode is not None:
                        node_id = mynode.state.transformer_uniqueid if mynode.state.transformer_uniqueid is not None else 'None'
                        node_id_str = node_id + '({})'.format(mynode.unique_node_id)
                        selnode_ids += node_id_str + ', '
                        mynode = mynode.orig_parent
                    attackinstance.logger.logger.debug("Selection node ids: " + selnode_ids)
        ...
        for ic, _ in sorted_indices_ic:
            if mcts_handler.mcts_statesequences[ic].final_score is not None:
                if self.wasattacksuccessfull(attackinstance=mcts_handler.inputlist[ic]) is True:
                    ...
                    if out is not None:
                        mcts_handler.inputlist[ic].logger.logger.debug("Expansion Sequence: ")
                        tf_seq = ''
                        for state in mcts_handler.mcts_statesequences[ic].states:
                            tf_seq += str(state.transformer_uniqueid) + ', '
                        mcts_handler.inputlist[ic].logger.logger.debug(tf_seq)
    ...
    def expand_node_single(self, attackinstance: BBAttackInstance, prefix: str, transformer_id, transformer_seed,
        ...
        result_string = " {}:`{}` successfull with {} ({}) [Before:{}({})]".\
            format(prefix, besttransfcall, np.round(scoreprednew, 4), classprednew,
                   np.round(outertempattackinstance.original_predscore, 4), self.tauthor.true_class)
