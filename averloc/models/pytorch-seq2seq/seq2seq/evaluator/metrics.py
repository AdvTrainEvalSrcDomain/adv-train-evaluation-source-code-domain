...
def calculate_metrics_from_files(pred_file, labels_file, orig_pred_file, verbose=False):
	f_pred = open(pred_file, 'r')
	f_true = open(labels_file, 'r')
	if orig_pred_file: o_pred = open(orig_pred_file, 'r')

	hypotheses = f_pred.readlines()
	references = f_true.readlines()
	orig_pred = o_pred.readlines() if orig_pred_file else None

	f_pred.close()
	f_true.close()

	a = calculate_metrics(hypotheses, references, orig_pred, verbose)
	for m in a:
		print('%s: %.3f'%(m,a[m]))
	print()
...
def calculate_metrics(y_pred, y_true, orig_y_pred=None, verbose=False, bleu=False):
	...
	asr_orig_exact_match_method_cnt, asr_not_exact_match_cnt = 0, 0
	...
	for i in a:
		...
		if orig_y_pred is not None:
			...
			if y_true[i].strip() == orig_y_pred[i].strip():
				asr_orig_exact_match_method_cnt += 1
			if y_true[i].strip() == orig_y_pred[i].strip() and y_pred[i].strip() != y_true[i].strip():
				asr_not_exact_match_cnt += 1
	...
	asr_method = round(asr_not_exact_match_cnt / asr_orig_exact_match_method_cnt * 100, 2) if orig_y_pred else None
...
def parse_args():
	...
	parser.add_argument('--o_pred', required=False)
...
if __name__=="__main__":
	args = parse_args()
	calculate_metrics_from_files(args.f_pred, args.f_true, args.o_pred, args.verbose)
