...
class MySequence(Sequence):
    def __init__(self, trainpath, testpath, prexmlpath, tfxmlpath, stylexmlpath, batch_size=128, n=None, filterbydict_orig_model=None, train_obj=None, finetune_sc=None):
        ...
        self.len = sum(len(files) for _, _, files in os.walk(os.path.join(self.trainpath)))
    ...
    def make_next_dataset_worker(self, lock, author, num_of_files, num_of_instances, orig_train_dir, transformed_progs, intra_epoch_ignore_dict, inter_epoch_ignore_dict):
        for prog in os.listdir(os.path.join(self.trainpath, str(self.epoch), author)):
            ...
            orig_prog_path = os.path.join(self.trainpath, author, prog)
    ...
    def make_next_dataset(self, patience=2):
        ...
        orig_train_dir = os.path.join(self.trainpath)
            ...
            if self.x is None:
                if self.filterbydict_orig_model is None:
                    ...
                    self.unigrammmatrix_train = StyloUnigramFeatures(inputdata=self.next_path,
                                                        ...
                                                        binary=False, tf=True, idf=True,
                                                        ngram_range=(1, 3), stop_words=stop_words_codestylo,
                                                        trainobject=self.train_obj.codestyloreference if self.train_obj is not None else None)
                    ...
                    unigrammmatrix_test = StyloUnigramFeatures(inputdata=self.testpath,
                                                                ...
                                                                binary=False, tf=True, idf=True,
                                                                ngram_range=(1, 3), stop_words=stop_words_codestylo,
                                                                trainobject=self.train_obj.codestyloreference if self.train_obj is not None and self.train_obj.codestyloreference is not None else self.unigrammmatrix_train)
                    testfiles: StyloFeatures = StyloFeaturesProxy(codestyloreference=unigrammmatrix_test)
                    self.train_obj, test_obj = learning._tfidf_feature_selection(train_obj=self.trainfiles, test_obj=testfiles,
                                                                   ...
                                                                   threshold=threshold, finetune_train_obj=self.train_obj)
                ...
                self.sc, x_train, self.x_test, y_train_c, y_test_c, feature_dim = preprocess.preprocess(num_classes, config_learning, x_train, x_test, self.y_train, self.y_test, self.finetune_sc)
