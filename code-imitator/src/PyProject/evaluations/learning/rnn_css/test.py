...
testattackpath = os.path.join(repo_path, "data", "test_cppgcj", "fold1")
...
unigrammmatrix_test = StyloUnigramFeatures(inputdata=testattackpath,
                                        ...
                                        noprogrammers=204, # FIXME #5: Change this to the number of authors in the testing set.
...
for i in range(testfiles.getfeaturematrix().shape[0]):
    ...
    # FIXME #6: Change how this author name extraction is done if you're testing on your own dataset.
    ...
    src_author = doclabels[i].split('.cpp')[0].split('_')[-1]
    if find_author(src_author) == predicted:
        ...
        if not gen_testset_list and testset_list != []:
            for item in testset_list:
                ...
                if doclabels[i].startswith(basename):
                    tp1+=1
                    total+=1
    ...
    #print(find_author(src_author))
    print(i, 'Program: ', doclabels[i], 'Predicted class: ', predicted)
    print(i, 'Source author: ', src_author, 'Predicted author: ', authors[predicted])
    #print(model.predict_proba(x_test[i], find_author(src_author)), model.predict_proba(x_test[i], predicted))

tp2=0
print(tp1, tp2)
print(total)
print('Accuracy: '+str(tp1 / total))
f1 = []
#print('tp', tp)
#print('fp', fp)
#print('fn', fn)
for i in range(204):
    ...
    f1.append(2*p*r / (p+r+0.0000001))
print('Avg F1: %2f' % (sum(f1) / len(f1)))

#print(trainfiles.getcolnames())
