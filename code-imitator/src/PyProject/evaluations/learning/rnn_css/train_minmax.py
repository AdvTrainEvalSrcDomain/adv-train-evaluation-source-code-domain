...
def train_step(fullnet_batch_num, x, y, doclabels_batch, authors_batch, width_mult_list):
    ...
    # backpropagate
    optimizer.apply_gradients(zip(sub_grads, model.trainable_variables))
...
if __name__ == '__main__':
    ...
    datasetpath = os.path.join(repo_path, "data", "trainnew_cpp_paper_minmax_comploss_fold1", "0")
...
# (The code for saving models has problem counting epoch#, so 301 actually means the 300th epoch's model will be saved)
    ...
    save_to = os.path.join(config_learning.learnmodelspath, 'minmax_checkpoints_' + random_str)
