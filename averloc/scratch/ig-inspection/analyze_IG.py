...
sys.stdout = Logger(os.path.join(opt.results_dir, 'output.out'))
...
l = ['add', 'assert', 'base', 'class', 'create', 'delete', 'do', 'file', 'find', 'get', 'handle', 'help', 
    'id', 'is', 'load', 'main', 'name', 'parse', 'path', 'read', 'remove', 'save', 'service', 'set', 'subsystem',
    'test', 'write', 'init']
filter_pred_words = set(l)

if not opt.recompute:
    try:
        data = json.load(open(os.path.join(opt.results_dir, 'data.json'), 'r'))
        print('Loaded data')
        print(len(data['normal']['IG_gini']))
    except:
        print('File load failed, recomputing...')
        opt.recompute = True
...
if opt.recompute:
    ...
    json.dump(data, open(os.path.join(opt.results_dir, 'data.json'), 'w'))
...
for pred_token in filter_pred_words:
    # print(pred_token)
    if pred_token not in data['normal']['input_tokens_map']:
        continue
    ...
    for model in models:
        ...
        l2 = []
        for x in t:
            if t[x]['count_IG'] >= min_freq:
                avg_IG = t[x]['sum_IG']/(t[x]['count_IG']+0.0000001)
                l1.append((avg_IG, x, t[x]['count_IG']))
            if t[x]['count_attn'] >= min_freq:
                avg_attn = t[x]['sum_attn']/(t[x]['count_attn']+0.0000001)
                l2.append((avg_attn, x, t[x]['count_attn']))
            # print(x, 'Avg IG:', avg_IG, 'Avg attn: ', avg_attn)
        l1 = sorted(l1, reverse=True, key=lambda x:x[2])[:topk]
...
for pred_token in to_tabulate:
    if pred_token not in filter_pred_words:
        continue

    if pred_token not in to_tabulate:
        continue

    c1.extend([pred_token])                 
    if 'IG' in to_tabulate[pred_token]['normal']:
        c1.append(data['normal']['output_gini'][pred_token]['count_IG'])
        tmp = ['%s (%.5f, %d)'%(x[1], x[0], x[2]) for x in to_tabulate[pred_token]['normal']['IG']]
        c2.extend(tmp+[' ']*(topk-len(tmp))+[sep])
    else:
        c1.append('-')
        c2.extend([' ']*topk+[sep])

    if 'IG' in to_tabulate[pred_token]['adversarial-all']:
        c1.append(data['adversarial-all']['output_gini'][pred_token]['count_IG'])
        tmp = ['%s (%.5f, %d)'%(x[1], x[0], x[2]) for x in to_tabulate[pred_token]['adversarial-all']['IG']]
        c3.extend(tmp+[' ']*(topk-len(tmp))+[sep])
    else:
        c1.append('-')
        c3.extend([' ']*topk+[sep])
    ...
    if 'attn' in to_tabulate[pred_token]['normal']:
        c1.append(data['normal']['output_gini'][pred_token]['count_attn'])
        tmp = ['%s (%.5f, %d)'%(x[1], x[0], x[2]) for x in to_tabulate[pred_token]['normal']['attn']]
        c4.extend(tmp+[' ']*(topk-len(tmp))+[sep])
    else:
        c1.append('-')
        c4.extend([' ']*topk+[sep])
    if 'attn' in to_tabulate[pred_token]['adversarial-all']:
        c1.append(data['adversarial-all']['output_gini'][pred_token]['count_attn'])
        tmp = ['%s (%.5f, %d)'%(x[1], x[0], x[2]) for x in to_tabulate[pred_token]['adversarial-all']['attn']]
        c5.extend(tmp+[' ']*(topk-len(tmp))+[sep])
    else:
        c1.append('-')
        c5.extend([' ']*topk+[sep])

    c1.extend([' ']*(topk-5)+[sep])
...
c1 = ['%s (%.5f)'%(x[1], x[0]) for x in to_tabulate['normal']['IG']]
c2 = ['%s (%.5f)'%(x[1], x[0]) for x in to_tabulate['adversarial-all']['IG']]
c3 = ['%s (%.5f)'%(x[1], x[0]) for x in to_tabulate['normal']['attn']]
c4 = ['%s (%.5f)'%(x[1], x[0]) for x in to_tabulate['adversarial-all']['attn']]
...
title = '%s - %s/%s'%('seq2seq',opt.results_dir.split(os.sep)[-2], opt.results_dir.split(os.sep)[-1])

fig = plt.figure()
ax = fig.add_subplot(111)
ax.set_aspect(2.0)
# plt.subplot(2,2,1)
plt.boxplot([[i for i in data[model]['IG_gini'] if not np.isnan(i)] for model in models], labels=[r'Nat', r'Adv-$Q^{1,5}$'], whis='range', vert=True, meanline=True)
plt.title(title)
plt.savefig(os.path.join(opt.results_dir, 'Gini (IG).pdf'))


fig = plt.figure()
ax = fig.add_subplot(111)
# plt.subplot(2,2,2)
plt.boxplot([data[model]['attn_gini'] for model in models], labels=[r'Nat', r'Adv-$Q^{1,5}$'], whis='range', vert=True, meanline=True)
plt.title(title)
plt.savefig(os.path.join(opt.results_dir, 'Gini (attn).pdf'))
...
l.append(l1)
plt.boxplot(l, labels=['dGINI (IG)'], whis='range', vert=True, meanline=True) #, showfliers=False)
plt.title(title)
plt.savefig(os.path.join(opt.results_dir, 'Delta Gini Plot (IG).pdf'))
...
plt.boxplot(l, labels=['dGINI (attention)'], whis='range', vert=True, meanline=True)
plt.title(title)

plt.savefig(os.path.join(opt.results_dir, 'Delta Gini Plot (attn).pdf'))
plt.show()
