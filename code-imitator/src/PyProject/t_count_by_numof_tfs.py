...
for log_path in all_log_path:
  with open(log_path, 'r') as f:
    ...
    if found:
      ...
      else: nosuccess_list.append(log_path.split('/')[-3] + '_' + log_path.split('/')[-4])
    else:
      f.seek(0, 0)
      for line in f:
...
with open('ll.txt', 'w') as f:
  for item in nosuccess_list: f.write(os.path.join(input_dir, 'mcts/aaa/3264486_5633382285312000_' + item + '_mcts_t_seed40.cpp') + '\n')